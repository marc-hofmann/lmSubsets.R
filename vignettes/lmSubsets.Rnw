...
\Abstract{%
  ...
  written in {\CPP} and provides an efficient implementation of all
...
degree of error.  The core of the package is written in {\CPP}.  The
...
functions act as wrappers around the {\CPP} library, and are declared
as
%
\begin{CodeInput}
...
The algorithms employ the QRD to compress the data into a square
$\code{nvar}\times\code{nvar}$ matrix; the initial cost of
constructing the QRD is negligible.  Data configurations with varying
sizes ($\code{nvar}=20,25,30,35,40$) and degrees of noise
