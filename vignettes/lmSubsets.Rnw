...
statistical investigation.  On the other hand, the computational
strategy can be optimized for a specific selection criterion when
solving the best-subset selection problem~\eqref{eq:best_subset}
directly, and the IC-based approach can lead to computational costs
that are drastically improved.
...
proven to lie within well specified bounds of the optimum.
...
available for the {\R} system for statistical computing~\citep{R} from
...
%
\begin{equation*}
  f(S)-f(V)\leq(1+\mathtt{tolerance})\cdot (f(S^*)-f(V))\textrm{ ,}
\end{equation*}
%
...
The LASSO~\citep{tibshirani:j_roy_stat_soc_b_met:96} provides an
alternative to exact model selection methods, with an emphasis on
regularization.  The function \fct{glmnet}~\citep{glmnet} computes an
entire regularization path and returns a sequence of sparse
estimators.  It is not IC-based; rather, it employs a modified
objective function that induces sparsity by penalizing the regression
coefficients, and is extremely efficient.
...
each data configuration, five datasets are simulated.  Average
execution times of \fct{lmSelect} and \fct{glmnet} are reported, as
well as the average number of matches -- i.e., the number of best
subsets correctly identified -- and the speedup of the LASSO.  If the
first two best models are found but not the third, then \code{(nbest)}
is $2$; if the second is not found either, then \code{(nbest)} is $1$;
the average value then is $1.5$.
...
\begin{table}
  ...
  \begin{tabular}{rr|RRRR}
    \toprule
    sigma & nvar & lmSelect & glmnet & (speedup) & (nbest) \\
...
@ 
    \bottomrule
  \end{tabular}}
  \caption{Average execution times (in seconds) of \fct{lmSelect} and
    \fct{glmnet}, as well as the speedup and the average number of
    hits scored by the lasso.}
  \label{tab:bmlasso}
\end{table}
